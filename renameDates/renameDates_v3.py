...
for brazFilename in os.listdir('.'):
    ...
    # Compõe o nome do arquivo em estilo americano
    amerFilename = beforePart + '-' + monthPart + '-' + dayPart + '-' + yearPart + afterPart

    # Obtém os paths absolutos completos dos arquivos
    absWorkingDir = os.path.abspath('.')
    brazFilename = os.path.join(absWorkingDir, brazFilename)
    amerFilename = os.path.join(absWorkingDir, amerFilename)
    # Renomeia os arquivos
    print(f'Renaming {brazFilename} to {amerFilename}...')
    shutil.move(brazFilename, amerFilename)
